...
public class ArrowTargetPractice : MonoBehaviour
{
    [Header("References")]
    public OVRHand hand;
    public GameObject arrowPrefab; // assigner le prefab de la flèche ici (ou un GameObject contenant un ParticleSystem)
    ...
    void StartCharging()
    {
        ...
        if (showChargingEffect)
        {
            ...
            // Instancier le prefab (peut être une flèche ou un GameObject contenant un ParticleSystem)
            GameObject go = Instantiate(arrowPrefab, chargingFireBall.transform) as GameObject;
            if (go != null)
            {
                go.transform.localPosition = Vector3.zero;
                go.transform.localRotation = Quaternion.identity;
                // essayer de récupérer un ParticleSystem pour appliquer la couleur
    ...
    void LaunchFireBall()
    {
        ...
        else
        {
            ...
            // Instancier le prefab (flèche ou GameObject avec ParticleSystem) comme enfant
            GameObject go = Instantiate(arrowPrefab, fireBall.transform) as GameObject;
            if (go != null)
            {
                go.transform.localPosition = Vector3.zero;
                go.transform.localRotation = Quaternion.identity;
